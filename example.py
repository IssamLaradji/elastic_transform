...
image = torchvision.transforms.ToTensor()(Image.open(fname))[None][:,[0]]

et = elastic_transform.ElasticTransform(alpha=500,  sigma=10, random_seed=42)
...
image_transformed_numpy = elastic_transform_numpy.elastic_transform_numpy(image.numpy().squeeze(), 
                                alpha=500, sigma=10, random_state=42)
